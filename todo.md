...
- [x] Globe Enhancement 1: Atmospheric glow (volumetric halo + day/night terminator)
- [x] Globe Enhancement 2: Arc enhancements (variable width, dash animation, comet tails, lightning flash on critical)
- [x] Globe Enhancement 3: Hex-bin heatmap layer (attack density aggregation per region)
- [x] Globe Enhancement 4: Orbital ring / satellite layer (sensor nodes at altitude)
- [x] Globe Enhancement 5: Connection clustering (botnet trunk-to-branch visualization)
- [x] Globe Enhancement 6: Terrain & topology (bump-mapped dark vector style globe)
- [x] Globe Enhancement 7: Camera choreography (cinematic orbital path for kiosk mode)
- [x] Globe Enhancement 8: Pulse rings on target cities (stacking concentric rings at impact)
- [x] Gap fix: Implement day/night terminator on globe (dynamic light/shadow boundary)
- [x] Gap fix: Add comet-tail afterimage effect for attack arcs (persist and fade)
- [x] Gap fix: Render orbital ring path around the globe connecting sensor nodes
